...
if (isset($_GET["editar"])) {
    $editando = true;
    $id = $_GET["editar"];
    $sql = "SELECT p.id_profesor, p.nombre, p.apellido, p.nivel, u.usuario, u.id as usuario_id
            FROM profesores p 
...
if (isset($_GET["eliminar"])) {
    $id = $_GET["eliminar"]; $conexion->query("DELETE FROM profesores WHERE id_profesor = $id"); 
}
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (!in_array($nivel, $niveles_validos)) {
        echo "<script>alert('El nivel ingresado no es válido. Debe ser Primero, Segundo, Tercero, Cuarto, Quinto o Sexto.'); window.location.href='gestionar_profesores.php';</script>";
        exit();
    }
    ...
    if ($resultado_usuario->num_rows == 0) {
        echo "<script>alert('El usuario asociado no existe en la tabla de usuarios.'); window.location.href='gestionar_profesores.php';</script>";
        exit();
    }

    if ($editando) {
        $sqlP = "UPDATE profesores SET nombre = ?, apellido = ?, nivel = ?, usuario_id = ? WHERE id_profesor = ?";
        $stmtP = $conexion->prepare($sqlP);
        $stmtP->bind_param("sssii", $nombre, $apellido, $nivel, $usuario_id, $_POST["id_prof"]);
    } else {
        $sqlP = "INSERT INTO profesores (nombre, apellido, nivel, usuario_id) VALUES (?, ?, ?, ?)";
        $stmtP = $conexion->prepare($sqlP);
...
<div class="container">
    <h2><?php echo $editando ? "Editar Profesor" : "Registrar Profesor"; ?></h2>

    <form method="POST">
        <?php if ($editando): ?>
            <input type="hidden" name="id_prof" value="<?php echo $datos_editar["id_profesor"]; ?>">
    ...
    <table border="1" width="100%">
        ...
        <?php
        $sql = "SELECT p.id_profesor, p.nombre, p.apellido, p.nivel, u.usuario 
                FROM profesores p 
                JOIN usuarios u ON p.usuario_id = u.id";
        ...
        while ($fila = $resultado->fetch_assoc()) {
            ...
            echo "<td>
                    <a href='gestionar_profesores.php?editar={$fila['id_profesor']}'>Editar</a> | 
                    <a href='gestionar_profesores.php?eliminar={$fila['id_profesor']}'
